resource "aws_ssm_parameter" "namespace_id" {
  ...
  type  = "String"
  value = lookup(local.created_namespace, var.namespace_type).id
}
...
resource "aws_ssm_parameter" "namespace_arn" {
  ...
  type  = "String"
  value =  lookup(local.created_namespace, var.namespace_type).arn
...
resource "aws_ssm_parameter" "namespace_hosted_zone_id" {
  count = var.create_ssm_parameters && (try(lookup(local.created_namespace, var.namespace_type).hosted_zone, null) != null) ? 1 : 0
  name  = "${var.ssm_parameter_prefix}/cloudmap/hostedzone/id"
  type  = "String"
  value = lookup(local.created_namespace, var.namespace_type).hosted_zone
}
